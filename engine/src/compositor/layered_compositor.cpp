...
CompositeResult LayeredCompositor::CompositeRegions(
    ...
    if (!reasons.empty()) {
        for (const auto reason : reasons) {
            res.fallback_reasons.push_back(FallbackReasonLabel(reason));
        }
        return res;
    }
    const auto t0 = std::chrono::steady_clock::now();
    if (!mixer_.MixRegions(input, dst, regions)) return res;
    const auto t1 = std::chrono::steady_clock::now();
    res.compose_ns =
        std::chrono::duration_cast<std::chrono::nanoseconds>(t1 - t0).count();
    res.ok = true;
