...
| Сценарий | Результат |
|---|---|
| Empty / cheap content | **True 50p уже есть** — `in_fps≈50`, `d_pairs≈125`/5с |
| `test1` 3ch DeckLink | **Потолок ~25 unique fps** — `d_pairs` avg ~1.6–2.8 (как Phase 17); `d_late=0`/`d_dropped=0` |
| Approach A (CEF dual BeginFrame in-flight) | **Опровергнут** — P0.2: 0% тиков с ≥2 OnPaint |
| Approach B (2 raster / 20 мс поле) | **Не обоснован** — P0.1: raster не ≤10 мс |
| Fallback (eager sequential packing) | Реализован; null без регрессии; на `test1` **не** поднимает `d_pairs` |

**Вывод:** weave/queue готовы к 50p; блокер — стоимость кадра Blink/Skia на сложном шаблоне + CEF coalescing. Дальше — Phase 19 (cost model / дешёвые шаблоны), не ещё один pump-трюк.

**Критерий успеха (исходный):** `in_fps=50` на test1 — **не выполнен**. Фаза закрыта с честной фиксацией потолка и безопасным Fallback-патчем.
