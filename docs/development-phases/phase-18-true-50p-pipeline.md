...
Требует пользователя на SDI (TV Logic): `test1` на 3 каналах в эфире после P3.2. Ожидание по метрикам: **заметной разницы плавности vs Phase 17 может не быть** (тот же ~25p-as-50i). Empty channel / простой `test` уже даёт true field pairs — там плавность должна быть выше.
