...
Это зашито в архитектуру движка (`decklink_consumer.cpp`): пара полей строится из одного приходящего от CEF кадра; движок запрашивает у CEF новый кадр только когда `paint_seq` изменился, иначе пара берётся из старого. Поэтому **целевой потолок `in_fps` = 25, а не 50**, пока не сделана архитектурная работа по «true 50p pipeline» (Phase 18 в [PERFORMANCE_ROADMAP.md](PERFORMANCE_ROADMAP.md)).
...
Поднять `in_fps` с 25 (текущий ceiling, см. §1.1.1 выше) до 50 — настоящее временное разрешение для 50i. Три возможных подхода (true interlace, half-render, 50p-as-50i) — оцениваются на Phase 18 после того, как Phase 15-17 сократят стоимость кадра. Зависит от 15, 16, 17.
