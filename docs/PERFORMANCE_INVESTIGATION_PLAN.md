...
- **Вопрос A — низкий FPS.** С реальным контентом движок упирается в ~17–18 fps (шаблон `test`) или ~25 fps (другой, более лёгкий шаблон дня 1) вместо целевых 50.
...
Самое чистое свидетельство из всех собранных данных — прямой контраст на живом стенде: пока шаблон не загружен (только 1×1px DOM damage-beacon, почти нулевая нагрузка на raster), движок **тривиально держит 50 fps**. Как только на всех трёх каналах включается шаблон `test`, FPS падает и стабилизируется на content-bound плато ~17–18 fps (день 2) или ~25 fps (день 1, другой, судя по всему более лёгкий шаблон). Потолок в 50 fps архитектурно достижим — конкретный контент шаблона слишком дорог для рендера при текущей реализации.
...
- В шаблоне `test` нет видео вообще — только примитивы, картинки, часы и заанимированные маски.
...
|---|---|
| Decode-хитч на loop видео в шаблоне | В `test` нет видео вообще |
...
# Часть III — Дальнейший план по Вопросу A (Phase 15-19)

```
Phase 15 (изоляция инвалидации beacon+контента) ──┐
Phase 16 (Performance Matrix, cost table)         ├──→ Phase 18 (soak validation) ──→ Phase 19 (Style Guide)
Phase 17 (насыщение raster-пула, "почему 60% CPU")┘
```

Фазы 15-17 независимы друг от друга и от Phase 14 (микрофризы) — могут вестись параллельно.

## Phase 15 — Изоляция инвалидации в отдельные composited layers

**Цель:** статичные части сцены (фон, неанимируемые в данный момент текстовые блоки) не должны перерастеризовываться каждый кадр — только реально анимируемые элементы.

- **15.1 Baseline:** прогнать `engine/research/parse-chrome-trace.mjs`/`parse-trace-internals.mjs` на живом канале с шаблоном `test`, посчитать Raster events/кадр и размер invalidation rect (текущие цифры Phase 12 — `24 DOM nodes, ~176 raster events/frame` — были получены на других бенчах, нужно свежее число именно для `test`).
- **15.2 Эксперимент:** `will-change`/layer promotion для beacon и для 1-2 статичных DOM-элементов `test`:
...
```
  ...
  Заменить механизм мигания beacon с `background` на `opacity`.
- **15.3 A/B измерение:** Raster events/кадр, `in_fps`, CPU% renderer, визуальная проверка на SDI. Явно сравнить `in_fps` «beacon-only» с «beacon-isolated + контент» — приближение ко второму значению к 50fps будет прямым доказательством эффективности.

**Критерий успеха:** снижение Raster events/кадр для статичных участков сцены, стабильный `in_fps=50` без визуальной регрессии на SDI.

## Phase 16 — Performance Matrix

Расширить `bench/bench-*.html` набор (`transform` vs `left`/`top`, варианты `clip-path`, текстовая плотность, `opacity`+`isolation`), прогнать через `engine/research/run-blink-research.sh`, свести в `docs/development-phases/phase-16-performance-matrix.md`.

## Phase 17 — Почему CPU используется на ~60%, а не на 100%

**Цель:** раз троттлинга нет, а контент content-bound — куда уходит оставшаяся мощность выделенных ядер? Два кандидата: (1) Chromium raster-пул не насыщает все 4 логических ядра канала; (2) часть времени кадра тратится не на CPU-bound работу, а на ожидание (IPC round-trip, синхронные точки в pump-цикле) — то есть узкое место может быть латентность, а не throughput.

- **17.1** Замерить количество активных raster-потоков: `ps -T -p $renderer_pid -o pid,tid,pcpu,stat,comm | sort -k3 -n -r` в реальном времени, смотреть на состояние `R` на пике нагрузки.
- **17.2** Явно задать `--num-raster-threads` (сейчас не выставлен, Chromium решает сам) — сравнить `in_fps`/CPU% до/после.
- **17.3** Расширить `--frame-log` (Phase 14.5) разбивкой: сколько микросекунд ушло на `CefDoMessageLoopWork()`, сколько — на сон в ожидании paint (`main.cpp:241-251`). Если pump большую часть тика спит — узкое место латентность, не raster-параллелизм.

**Критерий успеха:** количественный ответ — либо «raster-пул недонасыщен, увеличение параллелизма даёт X% прироста», либо «узкое место — латентность, добавление CPU не поможет».

## Phase 18 — Формальный длинный soak-тест по новому протоколу

Длительность 2+ часа, реалистичный шаблон `test`, обязательная фиксация времени каждого ручного действия оператора (протокол — Часть IV ниже). Критерий PASS: весь ряд `in_fps` без обвалов ниже 90% от content-bound потолка для данного шаблона (не от абстрактных 50, пока сам потолок не поднят Phase 15/16).

## Phase 19 — Style Guide и документация

`docs/TEMPLATE_PERFORMANCE_GUIDE.md` + обновление `.cursor/rules/` на основе выводов Phase 15/16.
...
1. **Закрыто**: есть два готовых шаблона — `test` (нагруженный, маски) и `test1` (тривиальный квадратик, X/Y-loop). Используются в Phase 14 по умолчанию: test1 для E1-E4, test для E7.
...
8. Нужно ли распространить итоговые фиксы (после Phase 15/16) не только на `test`, но и перепроверить на `test1` (тривиальный случай) — чтобы убедиться, что фиксы не ломают простейшие шаблоны?
...
Это измерение — прямое воспроизводимое опровержение тепловой гипотезы и одновременно количественное подтверждение content-bound природы Вопроса A (контент упирается в ~17-18 fps стабильно, без деградации во времени — если бы это была накопительная проблема, за 3+ часа она бы уже проявилась).
